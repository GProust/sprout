...
class StartupViewModel(private val repository: SproutRepository) : ViewModel() {
    ...
    val startup = combine(repository.parentProfile, repository.baby) { profile, baby ->
        val now = System.currentTimeMillis()
        when {
            profile == null -> Startup.Onboarding
            needsCheckIn(profile.lastCheckIn, now) ->
                Startup.CheckIn(profile.name, profile.role, baby?.name)
            else -> Startup.Main
        }
    }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), Startup.Loading)

    fun completeOnboarding(name: String, role: ParentRole, babyName: String, birthDate: Long) {
        viewModelScope.launch {
            ...
            repository.saveParentProfile(
                ParentProfileEntity(id = 1L, name = name.trim(), role = role, lastCheckIn = null),
            )
        }
    }

    fun submitMotherCheckIn(entity: MotherHealthEntity) {
        viewModelScope.launch {
            repository.addMotherHealth(entity)
